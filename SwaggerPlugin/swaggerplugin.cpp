...
bool SwaggerPlugin::initialize(const QStringList &arguments, QString *errorString)
{
    ...
    QAction *action = new QAction(tr("Swagger Action"), this);
    Core::Command *cmd = Core::ActionManager::registerAction(action, Constants::ACTION_ID,
                                                             Core::Context(Core::Constants::C_GLOBAL));
    cmd->setDefaultKeySequence(QKeySequence(tr("Ctrl+Alt+Meta+A")));
    connect(action, &QAction::triggered, this, &SwaggerPlugin::triggerAction);
...
void SwaggerPlugin::triggerAction()
{
    QMessageBox::information(Core::ICore::mainWindow(),
                             tr("Action Triggered"),
                             tr("This is an action from Swagger."));
}
